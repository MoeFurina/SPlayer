...
import { DesktopLyricsManager } from "@main/desktopLyrics/desktopLyricsManager";
...
class MainProcess {
  constructor() {
    ...
    // 主窗口是否被用户手动隐藏
    this.isMainWindowHidden = false;
    // 主代理
    this.mainServer = null;
    // 网易云 API
    this.ncmServer = null;
    // 桌面歌词管理器
    this.desktopLyricsManager = null;
    // Store
  ...
  async init() {
    ...
    // 初始化桌面歌词管理器
    this.desktopLyricsManager = new DesktopLyricsManager(
      () => this.mainWindow,
      this.store,
      () => this.isMainWindowTrulyVisible()
    );
  ...
  mainAppEvents() {
    app.whenReady().then(async () => {
      ...
      // 设置桌面歌词管理器的主窗口事件监听
      this.desktopLyricsManager.setupMainWindowListeners();
    });
  ...
  // 检查主窗口是否真正可见（未被手动隐藏）
  isMainWindowTrulyVisible() {
    return this.mainWindow && 
           !this.mainWindow.isDestroyed() && 
           this.mainWindow.isVisible() && 
           !this.isMainWindowHidden;
  }
  
  // 桌面歌词管理器访问（向后兼容）
  get desktopLyricsWindow() {
    return this.desktopLyricsManager?.window;
  }

  showDesktopLyricsWindow() {
    this.desktopLyricsManager?.show();
  }

  hideDesktopLyricsWindow() {
    this.desktopLyricsManager?.hide();
  }
  
  toggleDesktopLyricsWindow() {
    this.desktopLyricsManager?.toggle();
  }

  setDesktopLyricsAlwaysOnTop(flag) {
    this.desktopLyricsManager?.setAlwaysOnTop(flag);
    }

  isDesktopLyricsVisible() {
    return this.desktopLyricsManager?.isVisible() || false;
  }
}
